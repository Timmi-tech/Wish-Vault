...
import { submitWishes } from '@/services/api';
...
export default function SendWishesModal({ isOpen, onClose }: SendWishesModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await submitWishes(formData);
        ...
        {!isSuccess ? (
          <>
            ...
            <p className="text-sm text-navy/70 text-center mt-2">
              Send a heartfelt birthday message to Oreoluwa
            </p>

            {/* Form */}
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div>
                <label className="font-label text-xs text-navy mb-1.5 block">Your Name *</label>
                <input
                  type="text"
                  name="name"
                  value={formData.name}
              ...
              <div>
                <label className="font-label text-xs text-navy mb-1.5 block">Your Message *</label>
                <textarea
                  name="message"
                  ...
                  rows={5}
                  className={`w-full px-4 py-3 rounded-xl border ${errors.message ? 'border-coral' : 'border-navy/10'} focus:border-peach focus:outline-none focus:ring-2 focus:ring-peach/20 text-navy text-[15px] placeholder:text-navy/30 transition-all duration-200 resize-none`}
                />
          ...
          <div className="text-center py-4">
            ...
            <p className="text-sm text-navy/70 mt-2 max-w-[300px] mx-auto">
              Your wishes have been sent. Oreoluwa will surely appreciate them!
            </p>
